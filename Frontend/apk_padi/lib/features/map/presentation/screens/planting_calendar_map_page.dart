...
class PlantingCalendarMapPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PlantingCalendarMapPage> createState() => _PlantingCalendarMapPageState();
}

class _PlantingCalendarMapPageState extends ConsumerState<PlantingCalendarMapPage> {
  final MapController _mapController = MapController();

  final LatLng _currentCenter = const LatLng(-6.3271, 108.3254); // Indramayu default
    ...
    try {
      ...
      if (geojson != null && geojson['features'] != null) {
        ...
        for (final feature in features) {
          ...
          final parsedPolygons = _parseGeoJsonPolygon(geometry, districtId, districtName);
  ...
  List<Polygon> _parseGeoJsonPolygon(Map<String, dynamic> geometry, int districtId, String districtName) {
    ...
    if (type == 'Polygon') {
      if (coords.isNotEmpty) {
        ...
        final points = outerRing.map((p) => LatLng((p[1] as num).toDouble(), (p[0] as num).toDouble())).toList();
        polygons.add(
          Polygon(
            points: points,
    ...
    if (position != null) {
      ...
      if (resolved != null) {
        ...
        if (mounted && resolved.district != null) {
          _showCalendarBottomSheet(resolved.district!.id, resolved.district!.name);
        }
      }
    }
  }

  Future<void> _showCalendarBottomSheet(int districtId, String districtName) async {
          ...
          FlutterMap(
            ...
            options: MapOptions(
              ...
              onTap: (tapPosition, point) async {
                ...
                final regionApi = ref.read(regionApiServiceProvider);
                final resolved = await regionApi.resolveCoordinates(point.latitude, point.longitude);
            ...
            children: [
              ...
              if (_polygonList.isNotEmpty) PolygonLayer(polygons: _polygonList),
                        ...
                        child: DropdownButton<RegencyModel>(
                          ...
                          items: _regencies.map((reg) {
                            return DropdownMenuItem<RegencyModel>(
                              value: reg,
                              child: Text(reg.name, style: const TextStyle(fontWeight: FontWeight.w600)),
                            );
                          }).toList(),
                          onChanged: (val) {
                            if (val != null) {
                              setState(() {
                                ...
                                if (val.latitude != null && val.longitude != null) {
                                  _mapController.move(LatLng(val.latitude!, val.longitude!), 11.0);
                                }
                              });
            ...
            Positioned(
              ...
              child: Card(
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.navigation, color: Color(0xFF4ADE80), size: 18),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(
                          _resolvedUserLocation!.formattedAddress,
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        SizedBox(width: 8),
                        Text('Memuat batas wilayah...', style: TextStyle(fontSize: 12)),
                      ],
                    ),
                  ),
                ),
