...
class MarketplaceApiService {
  ...
  Map<String, dynamic>? _extractObject(dynamic responseData) {
    ...
    Map<String, dynamic> root = Map<String, dynamic>.from(responseData);
  ...
  Future<List<MarketListingModel>> fetchListings() async {
    try {
      final response = await _getJson('/market-listings');
      ...
      throw Exception(_errorMessage(e.response?.data));
    }
  }
  ...
  Future<List<CategoryModel>> fetchCategories() async {
    try {
      ...
      if (status >= 300) {
        throw Exception('Gagal mengambil kategori. HTTP $status.');
      }
  ...
  Future<MarketListingModel> getListing(int id) async {
    try {
      final response = await _getJson('/market-listings/$id');

      final status = response.statusCode ?? 0;

      if (status >= 300) {
        throw Exception('Gagal mengambil listing. HTTP $status.');
      }
      ...
      if (root == null) {
        throw Exception('Response listing bukan JSON object.');
      }

      dynamic data = root['data'];

      while (data is Map && data['data'] is Map) {
        data = data['data'];
      }

      if (data == null && root.containsKey('id')) {
        data = root;
      }

      if (data is! Map) {
        throw Exception(
          root['message']?.toString() ?? 'Data listing tidak ditemukan.',
        );
      }
  ...
  Future<MarketListingModel> createListing({
    ...
    try {
      ...
      if (root == null) {
        throw Exception('Response server bukan JSON object.');
      }

      if (root['success'] == false) {
        throw Exception(
          root['message']?.toString() ?? 'Gagal membuat listing.',
        );
      }

      final data = _extractObject(root);

      if (data == null) {
        throw Exception('Data listing tidak ditemukan.');
      }

      return MarketListingModel.fromJson(data);
    } on DioException catch (e) {
      debugPrint(
  ...
  Future<MarketOfferModel> createOffer({
    ...
    try {
      ...
      if (root['success'] != true) {
        throw Exception(
          root['message']?.toString() ?? 'Gagal mengirim penawaran.',
        );
      }
      ...
      return MarketOfferModel.fromJson(data);
    } on DioException catch (e) {
      throw Exception(_errorMessage(e.response?.data));
    }
  }
  ...
  Future<List<MarketOfferModel>> fetchMyOffers() async {
    try {
      final response = await _getJson('/market-offers');

      final status = response.statusCode ?? 0;

      if (status >= 300) {
        throw Exception('Gagal mengambil penawaran. HTTP $status.');
      }
  ...
  Future<List<MarketOfferModel>> fetchListingOffers(int listingId) async {
    try {
      final response = await _getJson('/market-listings/$listingId/offers');
  ...
  Future<MarketOfferModel> updateOfferStatus({
    ...
    try {
      ...
      if (counterQuantity != null) {
        payload['counter_quantity'] = counterQuantity;
      }
      ...
      if (root['success'] != true) {
        throw Exception(
          root['message']?.toString() ?? 'Gagal memperbarui penawaran.',
        );
      }
  ...
  Future<List<PurchaseContractModel>> fetchContracts() async {
    try {
      final response = await _getJson('/purchase-contracts');

      final status = response.statusCode ?? 0;

      if (status >= 300) {
        throw Exception('Gagal mengambil kontrak. HTTP $status.');
      }
  ...
  PurchaseContractModel _parseContractResponse(
    ...
    if (root == null) {
      throw Exception('Response kontrak bukan JSON object.');
    }
    ...
    if (root['success'] == false) {
      throw Exception(
        root['message']?.toString() ?? 'Faktur pembelian tidak dapat diambil.',
      );
    }
    ...
    final contractData = _extractObject(root);

    if (contractData == null) {
      throw Exception(
        root['message']?.toString() ?? 'Data faktur pembelian tidak ditemukan.',
      );
    }
    ...
    final returnedId = contractData['id'];

    if (returnedId != null) {
      final parsedId = int.tryParse(returnedId.toString());

      if (parsedId != null && parsedId != contractId) {
        throw Exception(
    ...
    return PurchaseContractModel.fromJson(contractData);
  }
  ...
  Future<PurchaseContractModel> getContract(int contractId) async {
     ...
     */
       ...
       */
        ...
        try {
          ...
          throw Exception('Server mengembalikan response bukan JSON valid.');
        }
      }
     ...
     */
      ...
      final contractJson = Map<String, dynamic>.from(data);
     ...
     */
      ...
      final contract = PurchaseContractModel.fromJson(contractJson);
      ...
      if (errorData is Map) {
        ...
        throw Exception(
          map['message']?.toString() ?? 'Gagal mengambil faktur pembelian.',
        );
      }
      ...
      throw Exception('Gagal mengambil faktur pembelian.');
    }
  }
  ...
  Future<PurchaseContractModel> createPurchaseContract({
    ...
    try {
      ...
      if (root['success'] != true) {
        throw Exception(
          root['message']?.toString() ?? 'Gagal membuat pesanan/kontrak.',
        );
      }
      ...
      return PurchaseContractModel.fromJson(data);
    } on DioException catch (e) {
      debugPrint(
  ...
  Future<Map<String, dynamic>> fetchSalesReport({String period = 'all'}) async {
    try {
      ...
      if (root == null) {
        throw Exception('Response laporan penjualan bukan JSON object.');
      }

      if (root['success'] != true) {
        throw Exception(
          root['message']?.toString() ?? 'Gagal memuat laporan penjualan.',
        );
      }
  ...
  Future<List<Map<String, dynamic>>> fetchContractPayments() async {
    try {
      final response = await _getJson('/contract-payments');
