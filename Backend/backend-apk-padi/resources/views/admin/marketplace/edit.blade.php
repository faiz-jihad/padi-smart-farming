...
<div class="market-page">
    ...
    <nav class="market-breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="market-breadcrumb-current">Edit Listing #{{ $listing->id }}</span>
    </nav>

    {{-- Header --}}
    <div class="market-header">
        <div>
            <h1 class="market-title">Edit Listing Produk #{{ $listing->id }}</h1>
            <p class="market-description">Perbarui informasi komoditas, harga, link transaksi, foto produk, atau status listing.</p>
        </div>
    ...
    <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:16px; padding:28px;">
        <form method="POST" action="{{ route('admin.marketplace.listings.update', $listing) }}">
            @csrf
            @method('PATCH')
            ...
            <div style="display:grid; grid-template-columns: 2fr 1fr 1fr; gap:20px; margin-bottom:20px;">
                <div>
                    ...
                    <input type="text" name="commodity" id="commodity" style="width:100%; padding:10px 14px; border:1px solid #cbd5e1; border-radius:10px; font-size:14px; box-sizing:border-box;" value="{{ old('commodity', $listing->commodity) }}" required>
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="quantity" id="quantity" style="width:100%; padding:10px 14px; border:1px solid #cbd5e1; border-radius:10px; font-size:14px; box-sizing:border-box;" value="{{ old('quantity', $listing->quantity) }}" required>
                </div>
                ...
                <div>
                    ...
                    <select name="unit" id="unit" style="width:100%; padding:10px 14px; border:1px solid #cbd5e1; border-radius:10px; font-size:14px; background:#fff;" required>
                        <option value="ton" @selected($listing->unit === 'ton')>Ton</option>
                        <option value="kg" @selected($listing->unit === 'kg')>Kg</option>
                        <option value="karung" @selected($listing->unit === 'karung')>Karung</option>
                    </select>
                </div>
            </div>
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px; margin-bottom:20px;">
                <div>
                    ...
                    <input type="number" step="100" name="price_per_unit" id="price_per_unit" style="width:100%; padding:10px 14px; border:1px solid #cbd5e1; border-radius:10px; font-size:14px; box-sizing:border-box;" value="{{ old('price_per_unit', (int) $listing->price_per_unit) }}" required>
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" style="width:100%; padding:10px 14px; border:1px solid #cbd5e1; border-radius:10px; font-size:14px; background:#fff;" required>
                        <option value="published" @selected($listing->status === 'published')>Published (Aktif Tampil)</option>
                        <option value="draft" @selected($listing->status === 'draft')>Draft</option>
                        <option value="closed" @selected($listing->status === 'closed')>Closed / Terjual</option>
                        <option value="expired" @selected($listing->status === 'expired')>Expired</option>
                    </select>
                </div>
            </div>
            ...
            <div style="background:#f0fdf4; border:1px solid #a7f3d0; border-radius:12px; padding:20px; margin-bottom:20px;">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <input type="url" name="sales_link" id="sales_link" style="width:100%; padding:10px 14px; border:1px solid #cbd5e1; border-radius:10px; font-size:14px; background:#fff; box-sizing:border-box;" placeholder="https://wa.me/6281234567890 atau https://tokopedia.com/produk-padi" value="{{ old('sales_link', $listing->sales_link) }}">
            ...
            <div style="margin-bottom:24px;">
                ...
                <textarea name="description" id="description" rows="3" style="width:100%; padding:10px 14px; border:1px solid #cbd5e1; border-radius:10px; font-size:14px; box-sizing:border-box;">{{ old('description', $listing->description) }}</textarea>
            </div>

            <div style="display:flex; gap:12px;">
                <button type="submit" class="btn-market-action btn-market-primary" style="padding:12px 24px; border:none; cursor:pointer;">Simpan Perubahan</button>
